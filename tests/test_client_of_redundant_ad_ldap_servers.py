...
class TestClientOfRedundantAdLdapServers(unittest.TestCase):
    ...
    def setUp(self):
        self.fake_server_list = [{'srvr-dc1.myad.private.example.com': {'port': 636,
                                                                        'ssl': True,
                                                                        'validate': True}},
        ...
        self.no_validate_list = [{'srvr-dc1.myad.private.example.com': {'port': 636,
                                                                        'ssl': True,
                                                                        'validate': False}},
                                 {'srvr-dc2.myad.private.example.com': {'port': 636,
                                                                        'ssl': True,
        ...
        self.no_ssl_list = [{'srvr-dc1.myad.private.example.com': {'port': 636,
                                                                   'ssl': False,
                                                                   'validate': False}},
                            {'srvr-dc2.myad.private.example.com': {'port': 636,
                                                                   'ssl': False,
    ...
    def test_client_ldap_auth_some_servers(self, mock_tls, mock_ldap3):
        a_client = ClientOfRedundantAdLdapServers(self.fake_server_list, "test")
        result = a_client.ldap_auth(ldap_uid="test", ldap_pass="1234")
        mocked_tls = mock_tls.return_value
        hostname = list(self.fake_server_list[0])[0]
        mock_ldap3.Server.assert_called_with(hostname,
                                             port=self.fake_server_list[0][hostname]['port'],
                                             use_ssl=self.fake_server_list[0][hostname]['ssl'],
                                             tls=mocked_tls)
        self.assertEqual(True, result)

    @mock.patch('client_of_redundant_servers.client_of_redundant_ad_ldap_servers.ldap3')
    def test_client_ldap_auth_unvalidated_servers(self, mock_ldap3):
        a_client = ClientOfRedundantAdLdapServers(self.no_validate_list, "test")
        result = a_client.ldap_auth(ldap_uid="test", ldap_pass="1234")
        hostname = list(self.no_validate_list[0])[0]
        mock_ldap3.Server.assert_called_with(hostname,
                                             port=self.no_validate_list[0][hostname]['port'],
                                             use_ssl=self.no_validate_list[0][hostname]['ssl'])
        self.assertEqual(True, result)

    @mock.patch('client_of_redundant_servers.client_of_redundant_ad_ldap_servers.ldap3')
    def test_client_ldap_auth_servers_without_ssl(self, mock_ldap3):
        a_client = ClientOfRedundantAdLdapServers(self.no_ssl_list, "test")
        result = a_client.ldap_auth(ldap_uid="test", ldap_pass="1234")
        hostname = list(self.no_ssl_list[0])[0]
        mock_ldap3.Server.assert_called_with(hostname,
                                             port=self.no_ssl_list[0][hostname]['port'])
        self.assertEqual(True, result)

    @mock.patch('client_of_redundant_servers.client_of_redundant_ad_ldap_servers.ldap3')
    def test_client_ldap_auth_some_servers_with_ad_domain(self, mock_ldap3):
        a_client = ClientOfRedundantAdLdapServers(self.fake_server_list, "test", ad_domain="testdomain")
        result = a_client.ldap_auth(ldap_uid="test", ldap_pass="1234")
